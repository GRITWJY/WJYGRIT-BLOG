...
function createSidebarData(sourceDir, collapsable) {
  ...
  tocs.forEach((toc) => {
    // toc是每个目录的绝对路径
    // toc =  '/Users/macbookair/Documents/WEBSTORM/YGRIT-BLOG-NEW/docs/01.前端'
...
function mapTocToSidebar(root, collapsable, prefix = "") {
  ...
  const files = fs.readdirSync(root); // 读取该目(文件和文件夹),返回数组
  files.forEach((filename) => {
    ...
    if (isNaN(order) || order < 0) {
      log(chalk.yellow(`warning:该文件"${title}序号出错，请填写正确的序号"`));
      return;
    }
