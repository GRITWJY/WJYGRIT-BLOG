  ...
  for (let i = 0, postsL = posts.length; i < postsL; i++) {
    ...
    if (type(categories) === "array") {
      ...
      function findCategory(obj, arr) {
        ...
        // 当前的存在了，就要判断是否需要进入children
        if (arr.length === 1) {
          obj[cur].children.push(post);
          // 此时已经走到最后了，直接return
          return;
        }
        // 如果arr还有，则要继续递归
        obj[cur].children && delete obj[cur].children;
        findCategory(obj[cur], arr.slice(1));
      }

      findCategory(categoriesObj, categories);
    }
...
export function categoriesAndTags(groupPosts) {
  ...
  function getCategories(obj, path) {
    for (let key in obj) {
      if (obj[key].children) {
        categoriesArr.push({
          key,
